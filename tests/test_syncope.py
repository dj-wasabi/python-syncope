...
def test_get_users_search_username():
    ...
    user_data = syn.get_users_search(search_req)
...
def test_get_users_username():
    ...
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.get_users_username("vivaldi")
    assert user_data['id'] == 3


def test_get_users_search_page():
    ...
    user_data = syn.get_users_search_page(search_req, 1, 1)
    username = user_data[0]['username']
    assert username == "rossini"


def test_edit_users_id_suspend():
    """Will suspend the user for user id 1.

    :return: Should return: suspended
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.edit_users_id_suspend(1)
    assert user_data['status'] == "suspended"


def test_edit_users_id_reactivate():
    """Will reactivate the user for user id 1.

    :return: Should return: active
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.edit_users_id_reactivate(1)
    assert user_data['status'] == "active"


def test_edit_users_username_suspend():
    """Will suspend the user for user username vivaldi.

    :return: Should return: suspended
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.edit_users_username_suspend("vivaldi")
    assert user_data['status'] == "suspended"


def test_edit_users_username_reactivate():
    """Will reactivate the user for user username vivaldi.

    :return: Should return: active
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.edit_users_username_reactivate("vivaldi")
    assert user_data['status'] == "active"
...
def test_delete_users_id():
    ...
    """
    syn = syncope.Syncope(syncope_url="http://192.168.10.13:9080", username="admin", password="password")
    user_data = syn.get_users_username("wdijkerman")
    user_id = int(user_data['id'])
    print str(user_id)
    assert syn.delete_users_id(user_id) == True
